...
export function CvUploadForm() {
  ...
  return (
    <form action={formAction} className="rounded-xl border border-slate-200 bg-white p-4 shadow-[0_1px_2px_rgba(15,23,42,0.04)] sm:p-5">
      <h2 className="text-lg font-semibold tracking-[-0.02em] text-slate-950">{t("cvs.upload.title")}</h2>
      <p className="mt-1 text-sm leading-6 text-slate-500">{t("cvs.upload.description")}</p>
      ...
      <div className="mt-5 grid gap-4 md:grid-cols-[1fr_1fr_auto] md:items-end">
        <label className="text-sm font-medium text-slate-700">
          ...
          <input className="mt-2 h-11 w-full rounded-lg border border-slate-200 bg-white px-3 text-sm text-slate-950 outline-none transition placeholder:text-slate-400 focus:border-indigo-500 focus:ring-3 focus:ring-indigo-100" maxLength={255} name="name" placeholder={t("cvs.upload.namePlaceholder")} required />
        </label>
        <label className="text-sm font-medium text-slate-700">
      ...
      <p className="mt-3 text-xs leading-5 text-slate-500">{t("cvs.upload.hint")}</p>
    </form>
  );
}
