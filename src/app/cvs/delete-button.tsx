...
export function CvDeleteButton({ cvId, cvName }: { cvId: string; cvName: string }) {
  ...
  async function confirmDelete() {
    ...
    try {
      ...
      if (!response.ok) throw new Error(typeof data.error === "string" ? data.error : t("cvs.deleteButton.errors.deleteFailed"));

      setConfirming(false);
      router.refresh();
    } catch (deleteError) {
      setError(deleteError instanceof Error ? deleteError.message : t("cvs.deleteButton.errors.deleteFailed"));
    } finally {
      setIsDeleting(false);
    }
  }
  ...
  return (
    <>
      <button
        aria-label={t("cvs.deleteButton.ariaLabel").replace("{name}", cvName)}
      ...
      {confirming ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/40 px-4">
          <div aria-labelledby="delete-cv-title" aria-modal="true" className="w-full max-w-sm rounded-xl border border-slate-200 bg-white p-5 shadow-xl" role="dialog">
            <h2 className="text-base font-semibold text-slate-950" id="delete-cv-title">{t("cvs.deleteButton.dialogTitle")}</h2>
            <p className="mt-2 text-sm leading-6 text-slate-600">
              {t("cvs.deleteButton.dialogDescriptionPrefix")}<span className="font-medium text-slate-800">{cvName}</span>{t("cvs.deleteButton.dialogDescriptionSuffix")}
            </p>
