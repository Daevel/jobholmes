...
export default async function CvsPage() {
  ...
  return (
    <AppShell accountLabel={user.name || user.email} currentPath="/cvs">
      <PageHeader subtitle={t("cvs.pageSubtitle")} title={t("cvs.pageTitle")} />
      <CvUploadForm />

      {cvs.length === 0 ? (
        <EmptyState description={t("cvs.emptyState.description")} title={t("cvs.emptyState.title")} />
      ) : (
        <SectionCard title={t("cvs.library.title")} description={t("cvs.library.description")}>
              ...
              <article key={cv.id} className="flex min-w-0 flex-col gap-4 rounded-lg border border-slate-200 bg-white p-4 sm:flex-row sm:items-center sm:justify-between">
                <div className="flex min-w-0 items-start gap-3">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="mt-1 text-xs text-slate-400">{t("cvs.library.uploadedPrefix")} {dateFormatter.format(cv.createdAt)} · {formatBytes(cv.sizeBytes)}</p>
                  </div>
                </div>
