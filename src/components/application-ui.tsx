...
function NavItem({ label, href, icon: Icon, currentPath }: { label: string; href: string; icon: LucideIcon; currentPath: string }) {
...
function MobileNavItem({ label, href, icon: Icon, currentPath }: { label: string; href: string; icon: LucideIcon; currentPath: string }) {
  const active = href === "/applications" ? currentPath === href || currentPath.startsWith("/applications/") : currentPath === href;
  ...
  return (
    <Link className={`flex flex-col items-center justify-center gap-1 rounded-lg px-2 py-2 text-xs font-medium outline-none transition focus-visible:ring-2 focus-visible:ring-indigo-500 ${active ? "text-indigo-700" : "text-slate-500"}`} href={href} aria-current={active ? "page" : undefined}>
      <span className={`flex h-8 w-8 items-center justify-center rounded-lg ${isAdd ? "bg-indigo-600 text-white" : active ? "bg-indigo-50" : ""}`}><Icon aria-hidden="true" className="h-4 w-4" /></span>
